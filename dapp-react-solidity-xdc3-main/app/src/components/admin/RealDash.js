...
import SidebarMenu from "./side.js";
 const { executeTransaction, queryData , log, EthereumContext} = require('react-solidity-xdc3');
...
function AdminDashBoard() {
  ...
  const regCompany = async(event) =>{
      ...
      const sendReward = async(event) =>{
        ...
        let resp = await executeTransaction(erc,provider,"sendReward",[employeeaddress,amount]);
      ...
      const isCompanyRegistered = async (event) => {
        ...
        let companyaddress = tokenn.wallet.replace("xdc", "0x");
        let response1 = await queryData(erc, provider, 'isCompanyRegistered',[companyaddress]);
  ...
  return (
    <div>
      <div className="row">
        ...
        <div style={{height:"150px", display:"flex",flexDirection:"row",marginTop: "0px", marginLeft: "-350px" ,backgroundColor:"#00F1C3"}}>
